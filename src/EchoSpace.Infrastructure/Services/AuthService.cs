    ...
    public class AuthService : IAuthService
    {
        ...
        private async Task HandleFailedLoginAttempt(User user)
        {
            ...
            if (user.AccessFailedCount >= maxAttempts)
            {
                ...
                _logger.LogWarning("Account locked for user {UserId} after {Attempts} failed attempts", user.Id, user.AccessFailedCount);
            }
        ...
        public async Task<bool> AdminUnlockAccountAsync(Guid userId)
        {
            ...
            if (user == null || !IsAccountLocked(user)) return false;
            
            user.AccessFailedCount = 0;
            user.LockoutEnd = null;
            await _context.SaveChangesAsync();
            
            _logger.LogInformation("Account unlocked for user {UserId} by admin", userId);
            return true;
        }
